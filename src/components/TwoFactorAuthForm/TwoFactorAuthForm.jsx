import React from 'react';

import Header from '../Header/Header';
import styles from './TwoFactorAuthForm.module.scss';
import { verifyTwoFactorCode } from '../../api/authApi';
import { useMutation } from '@tanstack/react-query';

const RESEND_TIMEOUT = 35;

const TwoFactorAuthForm = ({ onBack }) => {
  const [inputs, setInputs] = React.useState(['', '', '', '', '', '']);
  const [isAllInputsFilled, setIsAllInputsFilled] = React.useState(false);
  const [hasError, setHasError] = React.useState(false);
  const [isTimerExpired, setIsTimerExpired] = React.useState(false);
  const [timeLeft, setTimeLeft] = React.useState(RESEND_TIMEOUT);
  const [timerKey, setTimerKey] = React.useState(0);
  const [isRequestingNewCode, setIsRequestingNewCode] = React.useState(false);
  const inputRefs = React.useRef([]);

  React.useEffect(() => {
    setTimeLeft(RESEND_TIMEOUT);
    setIsTimerExpired(false);

    const interval = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          clearInterval(interval);
          setIsTimerExpired(true);
          return 0;
        }
        return prev - 1;
      });
    }, 1000);

    return () => clearInterval(interval);
  }, [timerKey]);

  const resetForm = () => {
    setInputs(['', '', '', '', '', '']);
    setIsAllInputsFilled(false);
    setHasError(false);
    inputRefs.current[0]?.focus();
  };

  const restartTimer = () => {
    setTimerKey((key) => key + 1);
  };

  const handleInputChange = (index, value) => {
    const normalized = value.replace(/\D/g, '').slice(0, 1);

    if (hasError) {
      setHasError(false);
    }

    setInputs((prev) => {
      const newInputs = [...prev];
      newInputs[index] = normalized;

      setIsAllInputsFilled(newInputs.every((input) => input.trim() !== ''));

      if (normalized && index < newInputs.length - 1) {
        inputRefs.current[index + 1]?.focus();
      }

      return newInputs;
    });
  };

  const handleKeyDown = (index, event) => {
    if (event.key === 'Backspace' && !inputs[index] && index > 0) {
      inputRefs.current[index - 1]?.focus();
      return;
    }

    if (event.key === 'Enter') {
      if (isAllInputsFilled && !hasError && !isTimerExpired) {
        handleSubmit();
      } else if (isTimerExpired && !isRequestingNewCode) {
        handleGetNewCode();
      }
    }
  };

  const verifyCodeMutation = useMutation({
    mutationFn: verifyTwoFactorCode,
    onSuccess: () => {
      alert('вы вошли в аккаунт');
      resetForm();
      restartTimer();
    },
    onError: () => {
      setHasError(true);
    },
  });

  const handleSubmit = () => {
    verifyCodeMutation.mutate({ code: inputs.join('') });
  };

  const handleGetNewCode = async () => {
    setIsRequestingNewCode(true);

    try {
      await new Promise((resolve) => setTimeout(resolve, 1000));
      alert('Новый код отправлен');
      resetForm();
      restartTimer();
    } finally {
      setIsRequestingNewCode(false);
    }
  };

  const shouldShowContinueButton = isAllInputsFilled && !isTimerExpired;
  const shouldShowGetNewButton = isTimerExpired;

  return (
    <div className={styles.twoFactorAuthForm}>
      <Header title="Two-Factor Authentication" />
      <div className={styles.desc}>Enter the 6-digit code from the Google Authenticator app</div>

      <div className={styles.inputWrapper}>
        {inputs.map((input, index) => (
          <input
            key={index}
            ref={(el) => (inputRefs.current[index] = el)}
            type="text"
            value={input}
            className={hasError ? styles.error : ''}
            onChange={(e) => handleInputChange(index, e.target.value)}
            onKeyDown={(e) => handleKeyDown(index, e)}
          />
        ))}
      </div>

      {shouldShowContinueButton && (
        <button
          className={`${styles.btn} ${hasError ? styles.invalidCode : ''}`}
          onClick={handleSubmit}
          disabled={hasError || verifyCodeMutation.isPending}>
          Continue
        </button>
      )}

      {shouldShowGetNewButton && (
        <button
          className={`${styles.btn} ${isRequestingNewCode ? styles.invalidCode : ''}`}
          onClick={handleGetNewCode}
          disabled={isRequestingNewCode}>
          {isRequestingNewCode ? 'Sending…' : 'Get new'}
        </button>
      )}

      {hasError && <div className={styles.error}>Invalid code</div>}

      <svg
        className={styles.arrowIcon}
        width="13"
        height="13"
        viewBox="0 0 13 13"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        onClick={onBack}
        role="button"
        tabIndex={0}
        onKeyDown={(event) => {
          if (event.key === 'Enter' || event.key === ' ') {
            onBack();
          }
        }}>
        <path
          d="M12.7973 5.58984H2.51238L8.66823 0.246094C8.76667 0.159961 8.70691 0 8.57683 0H7.02117C6.95261 0 6.88757 0.0246093 6.8366 0.0685546L0.193822 5.83242C0.132982 5.88516 0.0841885 5.95036 0.0507477 6.0236C0.0173069 6.09684 0 6.17642 0 6.25693C0 6.33745 0.0173069 6.41702 0.0507477 6.49027C0.0841885 6.56351 0.132982 6.62871 0.193822 6.68145L6.87527 12.4805C6.90163 12.5033 6.93327 12.5156 6.96667 12.5156H8.57507C8.70515 12.5156 8.76491 12.3539 8.66648 12.2695L2.51238 6.92578H12.7973C12.8747 6.92578 12.938 6.8625 12.938 6.78516V5.73047C12.938 5.65312 12.8747 5.58984 12.7973 5.58984Z"
          fill="black"
          fillOpacity="0.88"
        />
      </svg>
    </div>
  );
};
export default TwoFactorAuthForm;
